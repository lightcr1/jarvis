...
**2026-07-27 update — pluggable signal sources (V2 Phase 0):** `AlertEngine` no longer hardcodes its metric readers inline. `_default_signal_sources(ha_store) -> dict[str, Callable[[dict], float|str|None]]` in `jarvis/alert_engine.py` builds the built-in `{cpu, ram, disk, ha_health, ha_entity}` registry; `AlertEngine.__init__` stores it as `self._sources` (optionally merged with an `extra_sources` constructor kwarg), and `AlertEngine.register_source(metric: str, source: Callable[[dict], float|str|None]) -> None` lets later phases (Proxmox, NAS, etc.) add new metric readers post-construction without touching `_evaluate_rule` or the poll loop. `_read_metric` just does `self._sources.get(rule["metric"])(rule)` with try/except. Constructor signature (`rules_store`, `audit_admin_event`, `ha_store=None`, `broadcast_fn=None`) is unchanged/backward-compatible — existing tests that monkeypatch `engine._read_metric = lambda r: 99.0` still work since it's an instance-attribute override. See [[proactive-intelligence-v2]] for how this is used by the new suggestions engine (which does NOT plug into AlertEngine — it's a separate `SuggestionEngine` class; only alert *rules* would use `register_source`, e.g. a future `alerts.manage` rule with `metric: "proxmox.idle_vms"`).
