...
def build_calendar_router(deps: dict) -> APIRouter:
    ...
    def update_event(event_id: str, payload: dict[str, object], x_jarvis_session: str | None = Header(default=None)):
        session = deps["require_identity_session"](x_jarvis_session)
        force = bool(payload.get("force"))
        try:
            return current("calendar_service").update_event(
                event_id, payload, user_id=session["user"]["id"], role=session["user"]["role"], force=force,
            )
        except LookupError as exc:
