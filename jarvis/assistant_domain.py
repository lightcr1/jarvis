...
def _extract_city(t: str) -> str | None:
    m = re.search(
        ...
        r"(?=\s*\??$|\s+(?:right\s+now|please|bitte|currently|gerade|"
        r"today|heute|jetzt|now|tomorrow|morgen)|\s*$)",
        t, re.I,
    )
...
_FILE_DRIVE_DENIAL_TEMPLATE = _fd.DENIAL_TEMPLATE

_FILE_DRIVE_READ_PATTERNS = (
    re.compile(r"read\s+(.+?)\s+in\s+(?:my|this|the)\s+(.+?)\s+folder\.?$", re.I),
)

_FILE_DRIVE_LIST_ALL_PATTERNS = (
    re.compile(r"(?:what'?s|whats|what\s+is)\s+in\s+(?:my|this|these|the)\s+folders\??$", re.I),
    re.compile(r"(?:my|what)\s+folders(?:\s+do\s+i\s+have|\s+can\s+you\s+see)?\??$", re.I),
    re.compile(r"list\s+(?:my|the)\s+folders\.?$", re.I),
    re.compile(r"what\s+folders\s+are\s+(?:shared|granted)\??$", re.I),
)

_FILE_DRIVE_LIST_ONE_PATTERNS = (
    re.compile(r"(?:what'?s|whats|what\s+is)\s+in\s+(?:my|this|these|the)\s+(.+?)\s+folders?\.?\??$", re.I),
    re.compile(r"list\s+(?:my|this|the)\s+(.+?)\s+files?\.?$", re.I),
    re.compile(r"show\s+me\s+(?:my|this|the)\s+(.+?)\s+folder\.?\??$", re.I),
)


def _first_match(patterns, t: str):
...
def _handle_file_drive_skill(t: str, *, file_service, user_id: str) -> dict[str, object] | None:
    ...
    read_match = _first_match(_FILE_DRIVE_READ_PATTERNS, t)
    if read_match:
        filename = read_match.group(1).strip()
        folder_name = read_match.group(2).strip()
        folder = _fd.resolve_granted_folder(file_service, user_id, folder_name)
        if not folder:
    ...
    if _first_match(_FILE_DRIVE_LIST_ALL_PATTERNS, t):
        return _fd.list_all_folders(file_service, user_id)

    list_match = _first_match(_FILE_DRIVE_LIST_ONE_PATTERNS, t)
    if list_match:
