...
- **Mic error: `navigator.mediaDevices is undefined`**
  ...
  - Fix-Checkliste:
    1. UI über `https://<host>` **oder** lokal über `http://localhost:8000` öffnen (nicht über unsichere HTTP-Remote-URL).
    2. Browser-Mikrofonrechte erlauben (Site Settings → Microphone → Allow).
    3. In Chrome/Edge prüfen: `chrome://settings/content/microphone` und korrektes Input-Device wählen.
    4. Bei Reverse-Proxy HTTPS korrekt terminieren und `X-Forwarded-Proto=https` setzen.
    5. Gegenprobe: im gleichen Browser `navigator.mediaDevices` in der DevTools-Konsole prüfen.
