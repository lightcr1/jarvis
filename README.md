...
- `http://localhost:8000/static/orb-v2.html` (Orb-UI)
...
Damit `sudo systemctl start jarvis` nach dem Klonen funktioniert:
```bash
sudo scripts/install_service.sh
sudo systemctl start jarvis
```
Die Konfiguration liegt danach unter `/etc/jarvis/config.env` (falls noch nicht vorhanden, wird sie aus `config/jarvis.env.example` erstellt).
