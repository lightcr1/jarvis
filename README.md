...
### Installation
```bash
./scripts/install_sst.sh
```

Das Skript:
- prüft ob `sst` bereits vorhanden ist
- installiert SST global via `npm install -g sst` (falls `npm` verfügbar)
...
> Falls `npm`/Node fehlen: zuerst Node.js + npm installieren, dann `install_sst.sh` erneut ausführen.
...
- "Wake word" (`jarvis`, `hey jarvis`) führt jetzt direkt zu einem sinnvollen "online/ready"-Status statt unnötiger Ambiguität.
