      ...
      <div style={{ padding: '13px 0' }}>
        ...
        {showGuide && (
          <ol style={{ marginTop: 10, paddingLeft: 20, fontSize: 13, color: J.textSec, lineHeight: 1.7 }}>
            ...
            <li>In Stripe, go to <strong>Developers → Webhooks</strong>, add an endpoint pointing at <code>{`${window.location.origin}/webhooks/stripe`}</code>, and subscribe it to the <code>checkout.session.completed</code>, <code>customer.subscription.updated</code>, and <code>customer.subscription.deleted</code> events. Copy the endpoint's <strong>Signing secret</strong> (starts with <code>whsec_</code>) and paste it above.</li>
            <li>Click <strong>Test connection</strong> to confirm the secret key works. Users can now click "Subscribe" on the AI & Billing tab — Stripe handles checkout and recurring charges; JARVIS assigns the plan automatically when checkout completes, and removes it as soon as Stripe reports the subscription is no longer active (cancelled, or a failed renewal charge past Stripe's retry window).</li>
          </ol>
        )}
        <div style={{ marginTop: 12, fontSize: 12, color: J.textMuted }}>
          Note: JARVIS grants a plan's monthly AI credit once assigned and keeps granting it every calendar month as long as the plan stays assigned. It unassigns the plan the moment Stripe reports the subscription status is no longer "active" or "trialing" (covers cancellation and failed renewal charges, not just a hard delete) — make sure the webhook above is subscribed to <code>customer.subscription.updated</code>, or a lapsed payment won't be caught until Stripe eventually deletes the subscription outright.
        </div>
      </div>
    </>
  );
