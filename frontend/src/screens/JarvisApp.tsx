...
import { J, useJ, applyTheme, applyAccent, applyCompact, StatusBadge, ToastContainer, IconChat, IconOrb, IconHome, IconGrid, IconSettings, IconServer, IconX } from './jarvis-shared';
...
import { getSessionToken, clearStoredIdentity, getStoredPreferences, getStoredUser } from '../shared/api/client';
import { useJarvisAlerts } from '../shared/api/alerts';
import { useJarvisLiveStatus } from '../shared/api/status';

type Screen = 'login' | 'chat' | 'orb' | 'home' | 'proxmox' | 'services' | 'settings';
...
function NavRail({ current, onNav, onLogout }: { current: Screen; onNav: (s: Screen) => void; onLogout: () => void }) {
  ...
  const userInitial = ((prefs.display_name || user?.username || 'U')[0] ?? 'U').toUpperCase();
  return (
    <nav className="nav-rail" style={{ width: 60, flexShrink: 0, background: J.bg1, borderRight: `1px solid ${J.border}`, flexDirection: 'column', alignItems: 'center', padding: '14px 0', zIndex: 10, position: 'relative' }}>
        ...
        {NAV.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onNav(item.id)} title={item.label}
              ...
              onMouseEnter={e => { if (!active) { e.currentTarget.style.background = J.bg2; e.currentTarget.style.color = J.textSec; } }}
              onMouseLeave={e => { if (!active) { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = J.textMuted; } }}>
              <item.icon size={17} />
            </button>
          );
        })}
      ...
      <div style={{ position: 'relative' }}>
        ...
        {showMenu && (
          <>
            ...
            <div style={{ position: 'absolute', bottom: 40, left: 8, width: 160, background: J.bg2, border: `1px solid ${J.border}`, borderRadius: 10, zIndex: 21, overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
...
function BottomNav({ current, onNav }: { current: Screen; onNav: (s: Screen) => void }) {
  return (
    <nav className="nav-bottom" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 60, background: J.bg1, borderTop: `1px solid ${J.border}`, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom)' }}>
      {NAV.map(item => {
        const active = current === item.id;
        return (
...
export function JarvisApp() {
  ...
  const [screen, setScreen] = useState<Screen>(() =>
    getSessionToken() ? 'chat' : 'login'
  );
  // Show greeting on every fresh app open (not suppressed — fires each load by design)
  const [showGreeting, setShowGreeting] = useState(() => !!getSessionToken());
  ...
  const navigate = (s: string) => {
    const valid: Screen[] = ['chat', 'orb', 'home', 'proxmox', 'services', 'settings', 'login'];
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', paddingBottom: mobilePad ? 60 : 0, position: 'relative' }}>
        ...
        {screen === 'home'     && <HomeAssistantScreen />}
          ...
          {alerts.map(alert => (
            <div key={alert.id} style={{ width: 'min(340px, calc(100vw - 28px))', background: J.bg2, border: `1px solid ${alert.level === 'warning' ? J.warn : J.border}`, borderLeft: `3px solid ${alert.level === 'warning' ? J.warn : J.blue}`, borderRadius: 12, padding: '12px 14px', boxShadow: '0 12px 32px rgba(0,0,0,0.28)', pointerEvents: 'auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
                ...
                <button onClick={() => dismissAlert(alert.id)} style={{ background: 'none', border: 'none', color: J.textMuted, cursor: 'pointer', display: 'flex', padding: 0 }}>
                  <IconX size={14} />
                </button>
              </div>
